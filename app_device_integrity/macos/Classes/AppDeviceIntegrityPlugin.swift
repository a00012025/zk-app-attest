  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "getAttestationServiceSupport":
      guard let args = call.arguments else {
        result(FlutterError(code: "-1", message: "macOS could not extract " +
      ...
      guard let attest = AppDeviceIntegrity(challengeString: challengeString) else {
        result(FlutterError(code: "-1", message: "macOS could not initialize AppDeviceIntegrity", details: nil))
        return
      }
